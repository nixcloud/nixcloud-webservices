...
in rec {
  ...
  joinDomainSimple = lib.concatMapStringsSep "." escapeLabel;

  # Same as joinDomainSimple, but handle relative and absolute values
  # accordingly.
  joinDomainRelative = d: if d == [] then "@" else joinDomainSimple d;
  joinDomainAbsolute = d: "${joinDomainSimple d}.";
  ...
  joinDomain = domain:
    if domain ? absolute then joinDomainAbsolute domain.absolute
    else if domain ? relative then joinDomainRelative domain.relative
    else if domain == [] then throw "Empty domain passed to 'joinDomain'."
