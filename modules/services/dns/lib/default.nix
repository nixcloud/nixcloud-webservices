...
in rec {
  ...
  # Turn a splitted list of labels back into a single domain string.
  joinDomainSimple = domain: let
    emptyDomainErr = "Empty domain passed to 'joinDomainSimple'.";
    assertEmpty = x: if domain == [] then throw emptyDomainErr else x;
  in lib.concatMapStringsSep "." escapeLabel (assertEmpty domain);

  # Same as joinDomainSimple, but also takes care of domain.absolute and
  # domain.relative as generated by the "convertDomain" function below.
  joinDomain = domain: let
    # If the relative domain name is empty, we use the origin (@).
    relativeResult = if domain.relative == [] then "@"
                     else joinDomainSimple domain.relative;
    # We can't use joinDomainSimple here, as it doesn't allow empty domains.
    absoluteResult = lib.concatMapStringsSep "." escapeLabel domain.absolute;
  in if domain ? absolute then "${absoluteResult}."
     else if domain ? relative then relativeResult
     else joinDomainSimple domain;
