...
# https://codex.wordpress.org/Installing_WordPress#Things_to_Know_Before_Installing_WordPress
# http://www.wphostinggeeks.com/install-wordpress-ssh/
# https://code.tutsplus.com/articles/download-and-install-wordpress-via-the-shell-over-ssh--wp-24403

# NOTE: - after the service has been evaluated the first time a manual installation step has to be done in order to finish the installation
#       - wordpress has limited renaming support as it hardcodes the path somewhere: so moving from `foo.com/bar` to `bar.com/` is not trivial and requires
#         manual modifications of the mysql database and in case of plugin mailpoet isn't supported at all
...

{
  ...
  config = let
    ...
    # $wgDBtype = "${if (config.database.dbType == "postgresql") then "postgres" else config.database.dbType}";
    wordpressConfig = pkgs.writeText "wp-config.php" ''
      <?php
      define('DB_NAME',     '${config.database.dbName}');
      define('DB_USER',     '${config.database.dbUser}');
      define('DB_PASSWORD', '${config.database.dbPassword}');
      define('DB_HOST',     '${config.database.dbServer}');
    ...
    # FIXME / BUG this option isn't used for some reason
    # documentRoot = wordpressRoot; 

    database.dbType = mkOverride 900 "mysql";
    database.tablePrefix = mkOverride 900 "wp_";
